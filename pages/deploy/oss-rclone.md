...
而对于清除任务可通过**定时任务周期性删除 OSS 上的冗余资源**。由于该脚本定时完成，所以无需考虑性能问题，故不使用 `p-queue` 进行并发控制。

1. 使用 Linux 服务器中的 `crontab` 命令管理定时任务，每天凌晨两点清理，如果使用 Docker 部署，可将 crontab 脚本添加至镜像中。(需要注意单点问题，即某服务起多个容器，会清理多次，不建议如此使用)。
2. 使用 CI 配置定时任务清理，比如 Github Actions、Gitlab CI。

以下以 Github Actions 为例进行冗余文件定时删除。
...
1. 使用 rclone 上传文件至 oss/cos
1. 使用 rclone 部署自己的博客
1. 针对你们项目静态资源的存储及上传做了那些优化
